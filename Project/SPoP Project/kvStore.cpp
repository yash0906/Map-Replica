...
public:
    ...
    {
        // cout << "In get "<< key.data <<endl;
        char* wer = new char[key.size+1];
        ...
        {
        	// char* wer = new char[key.size];
        	for(int i=0;i<key.size;i++)
        	wer[i] = key.data[i];
        }
        // cout << "wer 	"<< sizeof(wer) << endl;
        ...
        NodePtr ptr = searchTreeHelper(this->root[lenght][index], wer);
    ...
    bool del(Slice &key)
    {
        ...
        if (deleteNodeHelper(this->root[lenght][index], wer, lenght, index) == true)
        {
            lextree[lenght][index]--;
            lexhash[lenght]--;
			pthread_mutex_unlock(&this->lock[lenght][index]);
    ...
    bool del(int n)
    {
        ...
        // cout << "deln " << key.data <<"  "<<key.size  <<endl;
        if (!success)
            return false;
        del(key);
        return true;
